...
<head>
 ...
 <body class="regis-bg"> <br><br><br>
    ...
    <div class="regis-box">
		<p id="text-lead">Enrolled</p>
        <?php
	        $con = con();
	        $sql = "SELECT * FROM class_enrollment WHERE user_id = '".$_SESSION['uid']."'";
	        if ($result = $con->query($sql)) {
		        while ($row = $result->fetch_assoc()) {
		        	$class = ongcon("SELECT * FROM class_main WHERE id = '".$row['class_id']."';");
		        	echo '<button name="form" class="ong-button" onclick="window.location.href=\'classroom_enrollment.php?id='.$row['class_id'].'\'">'.$class[0]['name'].'</button>';
		        }
		    } else {
		    	echo $con->error;
		    }
